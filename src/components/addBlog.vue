<template>
    <div id="add-blog">
        <h2> Add a New Blog Post</h2>
        <form v-if="!submitted">
            <label> Blog Title: </label>
            <input tyle="text" v-model.lazy="blog.title" required>
            <label>Blog Content: </label>
            <textarea v-model.lazy="blog.content"></textarea>
            <div id="checkboxes">
                <label>Python</label>
                <input type="checkbox" value="Python" v-model="blog.categories"/>
                <label>Javascript</label>
                <input type="checkbox" value="Javascript" v-model="blog.categories"/>
                <label>Java</label>
                <input type="checkbox" value="Java" v-model="blog.categories"/>
                <label>c++</label>
                <input type="checkbox" value="C++" v-model="blog.categories"/>
            </div>
            <select v-model="blog.author">
                <option value="" disabled selected>Select your option</option>
                <option v-for="author in authors" :key="author">{{author}}</option>
            </select> 
            <button v-on:click.prevent="post">Add Blog</button>
        </form>
        <div v-if="submitted">
        <h3>You data have been added</h3>
        </div>
        <div id="preview">
        <h3> Preview Blog</h3>
        <p>Blog title:{{blog.title}}</p>
        <p>Blog content:</p>
        <p>{{blog.content}}</p>
        <p>Blog Categories:</p>
        <ul>
            <li v-for="category in blog.categories" :key="category">{{category}}</li>
        </ul>
        </div>
        <p> Author: {{blog.author}} </p>
    </div>
</template>
<script>
export default {
    data(){
        return{
            blog:{title:'',content:'',categories:[],author:''},
            authors: ['Niranjan','SomeoneElse','Django'],
            submitted: false
        }
        
    },
    methods:{
        post(){
            this.$http.post('https://vuepractice-f4374.firebaseio.com/posts.json',this.blog).then(data=>{
                console.log(data)
                this.submitted=true;
                })
        }
    }
}
</script>
<style>
#add-blog *{
box-sizing: border-box;
}
#add-blog{
    margin: 20px auto;
    max-width:500px;
}

label{
    display: block;
    margin: 20px 0 10px;
}

input[type="text"], textarea{
    display:block;
    width: 100%;
    padding: 8px;
}

#preview{
    padding:10px 20px;
    border:1px dotted #ccc;
    margin:30px 0;
}

h3{
    margin-top:10px;
}
#checkboxes input{
    display: inline-block;
    margin-right:10px;
}

#checkboxes label{
    display:inline-block;
}
</style>
